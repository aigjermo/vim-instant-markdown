...
function! s:refreshView()
    ...
    call s:system("curl -X PUT -T - http://localhost:3000/ &>/dev/null &",
                \ s:bufGetContents(bufnr))
endfu

function! s:startDaemon(initialMD)
    call s:system("markmon -p 3000 --command \"pandoc --mathjax -t HTML5\" &>/dev/null &", a:initialMD)
endfu
...
function! s:killDaemon()
    call system("curl -s -X DELETE http://localhost:3000 / &>/dev/null &")
endfu
